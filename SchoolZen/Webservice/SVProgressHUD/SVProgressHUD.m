...
+ (void)showWithStatus:(NSString *)status networkIndicator:(BOOL)show {
    [[SVProgressHUD sharedView] showWithStatus:status maskType:SVProgressHUDMaskTypeClear networkIndicator:show];
}
...
- (UIWindow *)overlayWindow {
    if(!overlayWindow) {
        ...
        overlayWindow = [[UIWindow alloc] initWithFrame:CGRectMake(0, 65, frame.size.width, frame.size.height-65)];
